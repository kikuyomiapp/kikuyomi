...
All fifteen decisions in §9 of the architecture document have ADRs in `docs/adr/`. Fourteen are Accepted; ADR-0001, the QuickJS binding, is still Proposed. Its fork is vendored in `third_party/flutter_qjs`, and its probe runs on API 26 and API 35 emulators in CI (`.github/workflows/android-emulator.yml`, on pushes that touch the fork or the spike). The fork now uses a wall-clock interrupt deadline, since on Android `clock()` counted the whole process's CPU time. It stays Proposed until a three-line fork patch lands for the one failing probe: when the memory limit leaves no room for an error, QuickJS throws `null` and the host gets a Dart `TypeError`. Host calls with a string argument also restart the deadline, which the planned host-callback patch must fix.
